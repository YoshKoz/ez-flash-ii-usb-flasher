"""Merge loader_table2.bin patches into an2131_fw_v2.bin to see final code."""
from pathlib import Path

v2 = bytearray(Path("src/ezwriter-cli/an2131_fw_v2.bin").read_bytes())

# Parse loader_table2.bin
lt2 = bytearray(Path("src/ezwriter-cli/loader_table2.bin").read_bytes())
if lt2[:8] != b"EZWLDR1\0":
    print("Bad signature")
    exit(1)

count = u16 = lt2[8] | (lt2[9] << 8)
offset = 10
patched = v2[:]
patches = []
for i in range(count):
    addr = lt2[offset] | (lt2[offset+1] << 8)
    length = lt2[offset+2]
    data = lt2[offset+3:offset+3+length]
    patches.append((addr, bytes(data)))
    for j, b in enumerate(data):
        if addr + j < len(patched):
            patched[addr + j] = b
    offset += 3 + length

print(f"Loaded {count} patches, combined firmware = {len(patched)} bytes")

# Look at the combined cmd 0x02 handler
# Check what code is at the critical addresses
for label, addr in [("cmd dispatch 0x0E4A", 0x0E4A), ("cmd 0x02 handler 0x0DC5", 0x0DC5)]:
    print(f"\n=== {label} ===")
    hexbytes = " ".join(f"{patched[addr+i]:02x}" for i in range(min(32, len(patched)-addr)))
    print(f"  {hexbytes}")

# Check what patches cover the dispatch and handler area
print("\n=== Patches covering command handler area (0x0DB0-0x0E60) ===")
for addr, data in patches:
    end = addr + len(data)
    if addr <= 0x0E60 and end >= 0x0DB0:
        overlap_start = max(addr, 0x0DB0)
        overlap_end = min(end, 0x0E60)
        print(f"  addr=0x{addr:04X} len={len(data):3d} overlap=[0x{overlap_start:04X}-0x{overlap_end:04X}]")

# Full disassembly of patched cmd 0x02 handler
OPS = {}
ops_list = [
    ...
    (0x30,"JNB d,r",3),(0x31,"ACALL",2),(0x32,"RETI",1),(0x33,"RLC A",1),(0x34,"ADDC A,#i",2),
    (0x40,"JC r",2),(0x41,"AJMP",2),(0x42,"ORL d,A",2),(0x43,"ORL d,#i",3),(0x44,"ORL A,#i",2),
    (0x50,"JNC r",2),(0x51,"ACALL",2),(0x52,"ANL d,A",2),(0x53,"ANL d,#i",3),(0x54,"ANL A,#i",2),
    (0x60,"JZ r",2),(0x61,"AJMP",2),(0x62,"XRL d,A",2),(0x63,"XRL d,#i",3),(0x64,"XRL A,#i",2),
    (0x70,"JNZ r",2),(0x71,"ACALL",2),(0x73,"JMP @A+DPTR",1),(0x74,"MOV A,#i",2),(0x75,"MOV d,#i",3),
    ...
    (0x88,"MOV d,R0",2),(0x89,"MOV d,R1",2),(0x8A,"MOV d,R2",2),(0x8B,"MOV d,R3",2),(0x8C,"MOV d,R4",2),
    (0x8D,"MOV d,R5",2),(0x8E,"MOV d,R6",2),(0x8F,"MOV d,R7",2),
    ...
    (0xE5,"MOV A,d",2),(0xE8,"MOV A,R0",1),(0xE9,"MOV A,R1",1),(0xEA,"MOV A,R2",1),(0xEB,"MOV A,R3",1),
    (0xEC,"MOV A,R4",1),(0xED,"MOV A,R5",1),(0xEE,"MOV A,R6",1),(0xEF,"MOV A,R7",1),
    (0xF0,"MOVX @DPTR,A",1),(0xF1,"ACALL",2),(0xF5,"MOV d,A",2),(0xF8,"MOV R0,A",1),(0xF9,"MOV R1,A",1),
    (0xFA,"MOV R2,A",1),(0xFB,"MOV R3,A",1),(0xFC,"MOV R4,A",1),(0xFD,"MOV R5,A",1),
...
print("\n=== PATCHED firmware: cmd 0x02 handler @ 0x0DC5 ===")
disasm(patched, 0x0DC5, 60)

print("\n=== PATCHED firmware: dispatch @ 0x0E4A ===")
disasm(patched, 0x0E4A, 30)

# Also check the EP4 OUT interrupt handler entry
print("\n=== PATCHED firmware: EP4 OUT handler @ 0x15CE ===")
disasm(patched, 0x15CE, 30)
