...
# Format: "EZWLDR1\0" sig (8) + count (u16 LE) + records (addr u16 LE, len u8, data[len])
if data[:8] != b'EZWLDR1\0':
    raise SystemExit('Bad signature')
count = data[8] | (data[9] << 8)
print(f'Loader chunks: {count}')
...
print(f'Patched firmware size: {len(patched)} bytes')
print(f'Total bytes consumed from loader: {offset}')
...
ops_list = [
    ...
    (0x70,"JNZ r",2),(0x73,"JMP @A+DPTR",1),(0x74,"MOV A,#i",2),(0x75,"MOV d,#i",3),(0x76,"MOV @R0,#i",2),
    (0x77,"MOV @R1,#i",2),(0x78,"MOV R0,#i",2),(0x79,"MOV R1,#i",2),(0x7A,"MOV R2,#i",2),
    ...
    (0x8A,"MOV d,R2",2),(0x8B,"MOV d,R3",2),(0x8C,"MOV d,R4",2),(0x8D,"MOV d,R5",2),(0x8E,"MOV d,R6",2),(0x8F,"MOV d,R7",2),
    ...
    (0x9A,"SUBB A,R2",1),(0x9B,"SUBB A,R3",1),(0x9C,"SUBB A,R4",1),(0x9D,"SUBB A,R5",1),
    ...
    (0xF0,"MOVX @DPTR,A",1),(0xF2,"MOVX @R0,A",1),(0xF3,"MOVX @R1,A",1),
    (0xF5,"MOV d,A",2),(0xF6,"MOV @R0,A",1),(0xF7,"MOV @R1,A",1),
...
print('\n=== cmd 0x02 handler @ 0x0DC5 ===')
disasm(patched, 0x0DC5, 60)

print('\n=== dispatch @ 0x0E4A ===')
disasm(patched, 0x0E4A, 50)

print('\n=== Save type dispatch 0x0DDF area ===')
disasm(patched, 0x0DDF, 60)

print('\n=== 0x0F2A handler ===')
disasm(patched, 0x0F2A, 30)

print('\n=== EP4 OUT handler @ 0x15CE ===')
disasm(patched, 0x15CE, 50)

print('\n=== 0x190F / 0x1907 / 0x18D5 / 0x1917 areas ===')
disasm(patched, 0x1900, 48)

print('\n=== 0x0D9D (common LJMP target) ===')
disasm(patched, 0x0D9D, 40)
