"""Deep disassembly of tusbez.bin - extra addresses"""
    ...
    for _ in range(n):
        ...
        if size == 2 and pc+1 < len(data):
            ...
            if "LJMP" in mnem or "LCALL" in mnem:
                ...
                extra = f"  #0x{data[pc+1]:02X},#0x{data[pc+2]:02X}"
        lines.append(f"  {pc:04X}: {raw:<12} {mnem}{extra}")
        pc += size
    return lines

targets = {
    "0x0569 (cmd09 sub-target)": 0x0569,
    "0x0383 (cmd00 sub-target)": 0x0383,
    "0x03C7 (cmd05 sub-target)": 0x03C7,
    "0x0594 (cmd06 sub-target)": 0x0594,
    "0x0599 (error/nop handler)": 0x0599,
    "0x01AE (cmd01 path A)": 0x01AE,
    "CMD 0x01 continued @ 0x01A6": 0x01A6,
    "CMD 0x03 continued @ 0x021E": 0x021E,
}
